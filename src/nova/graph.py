...
from src.nova.my_mcp.config import mcp_config

from src.nova.prompts import ralph_system_prompt
import json,os
...
class  AgentState(BaseModel):
    ...
    protected_tools: List[str] = ["create_campaign", "send_campaign_email"] #confirm
...
async def build_graph():
    ...
    llm = ChatGoogleGenerativeAI(
        model="gemini-2.5-flash", google_api_key=os.getenv("GEMINI_API_KEYY")
    ).bind_tools(tools)
    ...
    def human_tool_review_node(state: AgentState) -> Command[Literal["assistant_node", "tools"]]:
        ...
        # Change the tool call arguments created by our Agent
           ## we may calll ai here to change the arguments
